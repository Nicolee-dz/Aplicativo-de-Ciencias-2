/**
 * Controlador — Búsqueda Secuencial Externa
 */
class ExternalSeqController {
    ...
    save() {
        ...
        doc.text('Búsqueda Secuencial Externa — Archivo guardado', 105, 27, { align: 'center' });
        ...
        blocks.forEach(block => {
            ...
            const row = block.records.join('   |   ');
            doc.setFontSize(9);
            doc.text(row, 25, y);
            y += 9;
        });

        // Marcador de datos para recuperar
        const marker = `##EXTSEQ_DATA##${JSON.stringify(arr)}##BLOCKSIZE##${this.model.blockSize}##END##`;
        doc.setFontSize(5);
        ...
        doc.save(`externo_secuencial_${Date.now()}.pdf`);
    ...
    recover(file) {
        ...
        file.arrayBuffer().then(buf => pdfjsLib.getDocument({ data: buf }).promise).then(async pdf => {
            ...
            const start = fullText.indexOf('##EXTSEQ_DATA##');
    ...
    print() {
        ...
        <title>Búsqueda Secuencial Externa</title>
        <style>
        ...
        <h2>Búsqueda Secuencial Externa</h2>
...
window.ExternalSeqController = ExternalSeqController;